...
use crate::{block, cpu, cpu::Registers, fd, page, process, vfs};
...
pub unsafe fn user_syscall(mepc: usize, frame_ptr: *mut cpu::TrapFrame) {
    ...
    match syscall_number {
        ...
        17 => {
            ...
            let bytes_to_copy = if size >= bytes.len() {
                bytes.len()
            } else {
                size
            };
            let bytes_copied =
                page::copy_to_user(process, buf, bytes.as_ptr() as usize, bytes_to_copy);
        ...
        214 => {
            // void *brk(void *addr)
            frame.regs[cpu::gpr(Registers::A0)] =
                process.set_brk(frame.regs[cpu::gpr(Registers::A0)]);
        }
        1024 => {
            ...
            if let Ok(entry) = op {
                ...
                process
                    .data
                    .fdesc
                    .insert(fd, fd::DescriptorType::File(entry));
            } else {
                fd = -1isize as u16;
            }
